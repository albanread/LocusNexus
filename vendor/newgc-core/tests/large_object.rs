...
use newgc_core::{
    Generation, HeapHeader, HeapType, LispLayout, PageHeap, PageKind, Tag, Word,
    G0_PROMOTION_THRESHOLD, G1_PROMOTION_THRESHOLD, PAGE_SIZE_CELLS, PAYLOAD_MASK,
};
...
fn alloc_large_object(h: &mut Heap, generation: Generation, n_cells: usize) -> Word {
    assert!(n_cells >= 1);
...
fn vm1_large_object_allocates_across_page_boundary() {
    ...
    assert!(
        ptr.is_some(),
        "alloc of PAGE_SIZE_CELLS+1 cells must succeed"
    );
    let ptr = ptr.unwrap();
    ...
    assert!(
        cont_desc.is_large_cont(),
        "second page must be a large cont"
    );
...
fn vm1_large_and_small_objects_coexist() {
    ...
    let mut small_root = [Word::from_ptr(small_ptr.as_ptr() as *const u8, Tag::Cons)];
...
fn vm1_table_buckets_size_24000_cells() {
    ...
    assert!(
        head.is_large_head(),
        "promoted page must still be a large head"
    );
...
fn vm1_large_payload_keeps_small_g0_child_alive_across_minor() {
    ...
    unsafe {
        let n_payload = (n_cells - 1) as u32;
        (large_ptr.as_ptr() as *mut u64).write(HeapHeader::new(HeapType::Vector, n_payload).raw());
    ...
    let large_word = Word::from_ptr(large_ptr.as_ptr() as *const u8, Tag::Vector);
    ...
    let payload_word = unsafe {
        let large_addr = (root[0].raw() & PAYLOAD_MASK) as *const u64;
