...
impl<L: HeapLayout> PageHeap<L> {
    pub(crate) fn gc_free_page_reserve_for_mutator_slab(&self) -> usize {
        ...
        self.page_count().div_ceil(4).min(self.page_count() - 1)
    }
    ...
    pub fn young_try_alloc_slab(&mut self, cells: usize) -> Option<(NonNull<u64>, usize)> {
    ...
    pub fn try_alloc_g0_slab(&mut self, cells: usize) -> Option<(NonNull<u64>, usize)> {
        if cells == 0 {
            return None;
        }
        let granted = cells.min(PAGE_SIZE_CELLS);
        self.try_alloc_g0_cons_slab(cells)
            .map(|slab| (slab, granted))
    }
    ...
    pub fn collect_minor_with_static(
        ...
        {
            let pin_g0 = self.pin_pointers_in_ranges(Generation::G0, pin_stack_ranges);
            let pin_g1 = self.pin_pointers_in_ranges(Generation::G1, pin_stack_ranges);
        ...
        let descs_at_scan_time: Vec<super::page_desc::PageDesc> = self.descs().to_vec();
    ...
    pub fn mark_minor_with_static(
...

        {
            ...
            scan_dirty_cards_as_marks(&mut marker, static_cards, static_base, static_cells, None);
    ...
    pub(crate) fn prepare_recycle_live_counts_from_marks(&mut self, target: Generation) {
    ...
    pub(crate) fn recycle_live_counts_active_for(&self, target: Generation) -> bool {
        self.recycle_live_counts_target == Some(target)
    }
    ...
    fn release_zero_live_unpinned_pages(&mut self, target: Generation) {
        let releasable: Vec<usize> = self
            .pages_in_gen(target)
            .filter(|&page_idx| {
                !self.desc(page_idx).has_pins() && self.recycle_live_counts[page_idx] == 0
            })
            .collect();
    ...
    fn try_alloc_g0_cons_slab(&mut self, cells: usize) -> Option<NonNull<u64>> {
        ...
        if self.count_pages_in_gen(Generation::Free) <= self.gc_free_page_reserve_for_mutator_slab()
        {
            return None;
        }
...
pub(super) fn clear_cards_unless_intergen<L: HeapLayout>(
    ...
    for card_idx in 0..card_idx_max {
        ...
        for c in card_start..card_end {
            ...
            if matches!(
                L::classify(cell),
                crate::traits::WordKind::PointerCons(_) | crate::traits::WordKind::PointerHeader(_)
            ) {
...
mod tests {
    ...
    fn mark_card_scan_must_not_treat_byte_payload_as_pointer() {
        ...
        let mut keep = {
            let p = h
                .try_alloc_boxed_in(Generation::G0, 4)
                .expect("string alloc");
            unsafe {
    ...
    fn slab_alloc_returns_aligned_pointer_and_advances_words_used() {
        ...
        let page = (slab.as_ptr() as usize - h.base_ptr() as usize) / (64 * 1024);
