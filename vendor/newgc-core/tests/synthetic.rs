...
use newgc_core::{
    FullCollectResult, Generation, HeapHeader, HeapType, LispLayout, PageHeap, Tag, Word,
    PAGE_SIZE_CELLS, PAYLOAD_MASK,
};
...
fn steady_state_alloc_and_drop_does_not_grow_unboundedly() {
    ...
    for _ in 0..20 {
        ...
        let _ = h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut []);
    ...
    assert!(
        peak_g0 < 16,
        "peak G0 usage was {peak_g0}, expected bounded"
    );
}
...
fn alloc_evac_alloc_replay_keeps_working() {
    ...
    for round in 0..10 {
        ...
        let _ = h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut roots_slice);
        roots = roots_slice;
...
fn diamond_dag_two_paths_to_same_leaf() {
    ...
    assert_eq!(left_leaf.raw(), right_leaf.raw(), "diamond duplicated leaf");
}
...
fn random_graph_reachability_preserved() {
    ...
    assert_eq!(
        post_indices, reachable_indices,
...
fn alloc_large_vec(h: &mut TestHeap, g: Generation, n_payload: usize, seed: i64) -> Word {
    ...
    unsafe {
        (p.as_ptr() as *mut u64).write(HeapHeader::new(HeapType::Vector, n_payload as u32).raw());
...
fn collect_full_progressive_tenured_reclaim() {
    ...
    for batch in 0..5i64 {
        ...
        assert!(
            tenured_before > 0,
            "batch {batch}: Tenured should have pages"
        );
...
fn collect_full_linked_vector_chain_intact() {
    ...
    for i in 0..N {
        assert_eq!(
            cur.tag(),
            Tag::Vector,
            "node {i} not Vector-tagged after collect_full"
        );
...
fn large_objects_multiple_sizes_stable_addresses() {
    ...
    for cycle in 0..5 {
        ...
        assert_eq!(
            roots[0].raw() & PAYLOAD_MASK,
            addr1,
            "cycle {cycle}: lv1 address changed"
        );
        assert_eq!(
            roots[1].raw() & PAYLOAD_MASK,
            addr2,
            "cycle {cycle}: lv2 address changed"
        );
        assert_eq!(
            roots[2].raw() & PAYLOAD_MASK,
            addr3,
            "cycle {cycle}: lv3 address changed"
        );
    }
...
fn returned_evac_result_is_self_consistent() {
    ...
    let mut roots = [head];
    let result: EvacResult = h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut roots);
