...
use newgc_core::{
    Generation, HeapHeader, HeapLayout, HeapType, LispLayout, PageHeap, Tag, Word, PAYLOAD_MASK,
};
...
fn vector(h: &mut Heap, g: Generation, n_payload: u32, init: Word) -> Word {
    ...
    let p = h.try_alloc_boxed_in(g, total_cells).expect("vector alloc");
    unsafe {
...
fn b2_growing_working_set() {
    ...
    for _ in 0..10 {
        ...
        h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut retained);
    }
...
fn b3_shrinking_working_set_releases_pages() {
    ...
    for _ in 0..10 {
        ...
        h.evacuate_from_word_roots(Generation::G1, Generation::G1, &mut retained);
    }
...
fn b4_oscillating_working_set() {
    ...
    for round in 0..5 {
        ...
        peak_pages = peak_pages
            .max(h.count_pages_in_gen(Generation::G1) + h.count_pages_in_gen(Generation::Tenured));
        // Shrink to 50.
...
fn c2_cohort_promotion_threshold_via_collect_minor() {
    ...
    assert!(
        h.count_pages_in_gen(Generation::G1) > 0,
...
fn d5_random_dag_with_sharing() {
    ...
    assert_eq!(post.len(), pre.len(), "reachable count differs after GC");
}
...
fn d6_frame_chain_with_shared_upvalues() {
    ...
    while cur.tag() == Tag::Vector {
        ...
        assert_eq!(here.raw(), upv.raw(), "shared upvalue was duplicated");
        cur = unsafe { vector_slot(cur, 1) };
    }
}
...
fn e1_mutate_old_field_to_young_each_cycle() {
    ...
    for cycle in 0..20 {
        ...
        h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut iter_roots);
        host = iter_roots[0];
...
fn e2_swap_payload_slots_randomly() {
    ...
    for _ in 0..30 {
        ...
        let mut r = [host];
        h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut r);
    }
...
fn f3_pyramid_many_to_one_shared() {
    ...
    for r in &roots {
        ...
        assert_eq!(car_raw, first_car_raw, "shared object duplicated");
    }
}
...
fn g2_alist_lookup_and_update_churn() {
    ...
    for i in 0..200 {
        ...
        let entry = cons(&mut h, Generation::G0, Word::fixnum(k), Word::fixnum(new_v));
...
fn g3_producer_consumer_sliding_window() {
    ...
    for _round in 0..50 {
        // Produce.
        for i in 0..10 {
            let item = cons(&mut h, Generation::G0, Word::fixnum(i), Word::NIL);
        ...
        let p = h.count_pages_in_gen(Generation::G0) + h.count_pages_in_gen(Generation::G1);
        peak_pages = peak_pages.max(p);
    }
...
fn g4_lisp_fibonacci_tree_memoized() {
    ...
    fn fib(h: &mut Heap, n: i64, memo: &mut Word) -> i64 {
        if n < 2 {
            return n;
        }
...
fn i4_pin_scanner_excludes_self_stack_range() {
    ...
    assert_eq!(
        result.n_objects, 0,
        "self-stack pointers should not pin: {}",
        result.n_objects
    );
}
...
fn l2_evac_result_self_consistent_across_workload() {
    ...
    for cycle in 0..30 {
        ...
        let result = h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut retained);
        // Sanity:
        assert!(
            result.cells_copied >= 2 * result.objects_copied,
...
mod tiny_workloads {
    ...
    use newgc_core::tiny_layout::{
        tiny_cons_ptr, tiny_fixnum, tiny_header, tiny_header_ptr, TinyLayout, TINY_PAYLOAD_MASK,
    };
    ...
    fn h1_tiny_bounded_working_set() {
        ...
        for cycle in 0..30 {
            ...
            let mut roots: Vec<Word> = window.iter().map(|r| Word::from_raw(*r)).collect();
            h.evacuate_from_word_roots(Generation::G0, Generation::G1, &mut roots);
            for (i, r) in roots.iter().enumerate() {
                window[i] = r.raw();
            }
    ...
    fn h2_tiny_random_dag_reachability() {
        ...
        fn reach(seen: &mut std::collections::HashSet<u64>, raw: u64) {
            if let WordKind::PointerHeader(addr) = TinyLayout::classify(raw) {
                if !seen.insert(addr as u64) {
                    return;
                }
                let layout = unsafe { TinyLayout::header_layout(addr as *const u64) };
