...
/// Cooperative-interrupt flag. A JIT'd Locus program runs to completion
/// on the worker thread with no safepoints, so the IDE cannot *preempt*
/// it — but a graphical program polls the event channel every frame.
/// Break/Restart set this flag; the very next `next_event` (i.e. the
/// program's own `poll_event`) consumes it and returns a synthetic
/// `Close`, so the program leaves its loop on its own, the `eval`
/// returns, and the worker is free to handle Restart / the next run.
/// Programs that never poll (a pure compute loop) still can't be
/// interrupted — that's the safepoint-free limit, documented as such.
static INTERRUPT_REQUESTED: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

/// Request a cooperative interrupt of the running program. Called on the
/// GUI thread from Break / Restart; observed on the worker thread by the
/// next `next_event`. Idempotent — coalesces if pressed repeatedly.
pub fn request_interrupt() {
    INTERRUPT_REQUESTED.store(true, std::sync::atomic::Ordering::SeqCst);
}

/// Consume a pending interrupt request, if any. Returns the synthetic
/// event a polling program should see (a child-0 `Close`).
fn take_interrupt() -> Option<IGuiEvent> {
    if INTERRUPT_REQUESTED.swap(false, std::sync::atomic::Ordering::SeqCst) {
        Some(IGuiEvent::Close { child_id: 0 })
    } else {
        None
    }
}
...
pub enum IGuiEvent {
    ...
    /// "Statically analyze this buffer (don't run it)." Fired from the
    /// Locus menu (Locus → Analyze) or F6. The pane snapshots its buffer
    /// (or selection) and pushes the text; the worker elaborates it (no
    /// JIT, no side effects, no event loop) and renders an effect/
    /// capability *report* into the doc pane — the powers the program
    /// names, drawn, without executing it.
    AnalyzeBuffer {
        source: String,
    },
    /// "Show a compiler-lowering view of this buffer." Fired from the Locus
    /// menu (ANF IR / LLVM IR / Assembly). The pane snapshots its buffer; the
    /// worker runs the front-end pipeline + lowering and renders the dump into
    /// the doc pane. `kind`: 0 = ANF IR, 1 = LLVM IR, 2 = assembly.
    ShowLowering {
        source: String,
        kind: i64,
    },
...
fn matches_filter(ev: &IGuiEvent, filter: &HashSet<i64>) -> bool {
    match ev {
        IGuiEvent::FrameClose | IGuiEvent::ThemeChange | IGuiEvent::EvalBuffer { .. } | IGuiEvent::AnalyzeBuffer { .. } | IGuiEvent::ShowLowering { .. } | IGuiEvent::ForthRestart | IGuiEvent::ForthInterrupt => true,
        IGuiEvent::Menu { .. } => true,
...
pub fn next_event(timeout_ms: i64) -> Option<IGuiEvent> {
    // A cooperative interrupt outranks everything: deliver the synthetic
    // Close immediately so a polling program exits its loop this frame.
    if let Some(ev) = take_interrupt() {
        return Some(ev);
    }
