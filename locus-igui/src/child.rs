...
/// Upload a `w×h` BGRA8 CPU buffer to a Direct2D bitmap and draw it at `(x, y)`.
/// The GUI-thread end of the shared-framebuffer boundary: one `CreateBitmap`
/// (bulk upload) + one `DrawBitmap`, replacing what would otherwise be `w*h`
/// per-pixel commands.
fn blit_pixels(
    target: &ID2D1HwndRenderTarget,
    x: f32,
    ...
    for (index, cmd) in batch.cmds.iter().enumerate() {
        match cmd {
            ...
            SurfaceCmd::Blit { x, y, w, h, pixels } => eprintln!(
                "[igui-batch-ui]   #{index} Blit ({x:.1}, {y:.1}) {w}x{h} ({} px)",
                pixels.len()
            ),
        }
    }
...
pub fn register_classes() -> Result<(), IGuiError> {
    ...
    // stack_view excised for Phase 0.
