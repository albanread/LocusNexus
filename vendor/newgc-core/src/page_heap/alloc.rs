...
fn align_up_cells(offset: usize, align_cells: usize) -> usize {
    debug_assert!(align_cells > 0);
    let rem = offset % align_cells;
    if rem == 0 {
        offset
    } else {
        offset + (align_cells - rem)
    }
}
...
impl<L: HeapLayout> PageHeap<L> {
    ...
    pub(crate) fn acquire_free_page(
        ...
        *self.desc_mut(idx) = PageDesc::fresh(generation, kind);
        ...
        unsafe {
            std::ptr::write_bytes(self.page_ptr(idx), 0, super::space::PAGE_SIZE_BYTES);
        }
        Some(idx)
    }
    ...
    fn try_alloc_in_region(
        ...
        if let Some(ptr) = self.try_bump_in_current(generation, kind, n_cells, is_cons, align_cells)
        {
            return Some(ptr);
        }
        if generation == Generation::G0
        ...
        region_ref.offset = 0;
        self.try_bump_in_current(generation, kind, n_cells, is_cons, align_cells)
    }
    ...
    fn try_bump_in_current(
        ...
        let (page_idx, alloc_offset, aligned_offset) = {
            let r = self.alloc_region(generation, kind);
            let aligned_offset = align_up_cells(r.offset, align_cells);
            if !r.has_page() || aligned_offset + n_cells > PAGE_SIZE_CELLS {
                return None;
            }
            (r.current_page, r.offset, aligned_offset)
        };
        ...
        let global_cell_idx = self.global_cell_index(page_idx, aligned_offset);
        let charged_cells = aligned_offset + n_cells - alloc_offset;

        // Update bookkeeping: advance region offset, advance
        // page's words_used, set start bit on Boxed.
        {
            let r = self.alloc_region_mut(generation, kind);
            r.offset = aligned_offset + n_cells;
        }
        {
            let d = self.desc_mut(page_idx);
            d.words_used = d.words_used.saturating_add(charged_cells as u16);
        }
        if is_cons {
        ...
        self.shared
            ...
            .fetch_add(charged_cells * 8, Ordering::Relaxed);
    ...
    pub fn try_alloc_large(
        ...
        for i in 0..n_pages {
            ...
            self.commit_page(idx).ok()?;
            let mut d =
                super::page_desc::PageDesc::fresh(generation, super::page_desc::PageKind::Large);
...
mod tests {
    ...
    #[test]
    fn boxed_allocs_are_16_byte_aligned() {
        let mut h = small_heap();
        for cells in [1, 2, 3, 5, 8, 13] {
            let p = h.try_alloc_boxed_in(Generation::G0, cells).unwrap();
            let addr = p.as_ptr() as usize;
            assert_eq!(addr % 16, 0, "boxed object must be 16-byte aligned");
        }
    }
