...
impl<'a, L: HeapLayout> PageMarker<'a, L> {
    ...
    pub(crate) fn new_without_clear(heap: &'a mut PageHeap<L>, target: Generation) -> Self {
        Self {
            heap,
            target,
            queue: Vec::new(),
...
impl<L: HeapLayout> PageHeap<L> {
    ...
    /// onto the queue.
    fn try_mark_root(&mut self, w: Word, target: Generation, queue: &mut Vec<usize>) {
    ...
    fn scan_marked_object(&mut self, cell_idx: usize, target: Generation, queue: &mut Vec<usize>) {
        ...
        for c in (cell_idx + layout.pointer_cells_start)..(cell_idx + layout.pointer_cells_end) {
            let w = Word::from_raw(self.read_cell(c));
            self.try_mark_root(w, target, queue);
        }
    }
    ...
    pub fn extend_mark_from_cross_gen_pinned(&mut self, target: Generation) -> usize {
        ...
        for cell_idx in pinned {
            ...
            let is_cons = is_cons_start_at(marker.heap.start_bits_slice(), cell_idx);
            let (payload_start, payload_end_inclusive) = if is_cons {
                (cell_idx, cell_idx + 1)
            } else {
                let header_ptr = unsafe { (marker.heap.base_ptr() as *const u64).add(cell_idx) };
                let layout = unsafe { L::header_layout(header_ptr) };
                if layout.pointer_cell_count() == 0 {
                    continue;
                }
            ...
            for c in payload_start..=payload_end_inclusive {
                ...
                marker.heap.try_mark_root(w, target, &mut marker.queue);
...
mod tests {
    ...
    fn marking_a_g0_root_doesnt_touch_g1_pages() {
        ...
        let g1_word = Word::from_ptr(g1_cons.as_ptr() as *const u8, Tag::Cons);
        ...
        let g0_word = Word::from_ptr(g0_cons.as_ptr() as *const u8, Tag::Cons);
    ...
    fn marking_a_boxed_object_walks_its_payload() {
        ...
        let target_word = Word::from_ptr(target.as_ptr() as *const u8, Tag::Cons);
        unsafe {
