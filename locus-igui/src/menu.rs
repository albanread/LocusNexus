...
pub fn install_for_frame(frame: HWND, mdi_client: HWND, spec: &str) -> bool {
    ...
    // is currently active.
    // Recent Files is populated by the default-menu path (`window::install_menu`);
    // a CP-set custom menu shows an empty list here.
    ...
    // RunIt entries are re-discovered each time we re-install the menu
    // (cheap dir scan); ensures user-added programs appear without
    // an IDE restart if the language thread pushes a new menu.
    let runit_files = super::window::runit_files_snapshot();
    let runit_pairs: Vec<(u16, String)> = runit_files
        .iter()
        .map(|(id, name, _)| (*id, name.clone()))
        .collect();
    super::tools_menu::append_runit_menu(menu, &runit_pairs);
