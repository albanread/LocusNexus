//! help_pane — the **Help / manual** pane: an MDI child that browses a
//! folder of Markdown documents, with the DocCrate-style sidebar — a list
//! of pages on the left, the current page on the right, a draggable
//! divider to resize the sidebar, and a ‹/› toggle (with a reveal tab) to
//! collapse and restore it. Click a sidebar row or an in-page link to
//! navigate; wheel to scroll.
//!
//! Specialised for help docs — deliberately **separate** from `doc_pane`
//! (which renders one-off *reports*). Ported from NewFactor's igui
//! help_pane; the open path is adapted to RNIM (we create the child on the
//! GUI thread directly from the Help → Documentation handler, and resolve
//! the docs folder ourselves). The content under `help/` is minimal for
//! now and will be expanded.
//!
//! The render core renders one document from text; this host owns the
//! browser chrome — folder scan, the row sidebar, navigation — drawn with
//! `docpane::render`'s `fill_rect` / `draw_text` primitives plus
//! `draw_document` for the page body. The pane's render target comes from
//! *docpane's* D2D factory (`render::factory()`), so Mermaid geometry and
//! the target share one factory.
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    RegisterClassExW, SendMessageW, SetCursor, SetWindowLongPtrW, CREATESTRUCTW, CW_USEDEFAULT,
    GWLP_USERDATA, IDC_ARROW, IDC_HAND, IDC_SIZEWE, MDICREATESTRUCTW, WM_LBUTTONDOWN, WM_LBUTTONUP,
    WM_MDIACTIVATE, WM_MDICREATE, WM_MOUSEMOVE, WM_MOUSEWHEEL, WM_NCCREATE, WM_NCDESTROY, WM_PAINT,
...
const HELP_CLASS: PCWSTR = w!("WF64.iGui.HelpPane");
...
/// HWND of the singleton Help child, when one is open. The Help pane is
/// one-at-a-time: a second open() activates the existing window instead of
/// spawning another.
static HELP_HWND: std::sync::Mutex<Option<isize>> = std::sync::Mutex::new(None);
...
/// Resolve the docs folder the Help pane browses. Mirrors the RunIt scan:
///   1. `<exe>/help`               — shipped beside the exe (release/help)
///   2. `<exe>/../../../help`      — dev: exe under target/<profile>/
///   3. `CARGO_MANIFEST_DIR/../help` — repo-root help/ (locus-igui/..)
/// Returns the first directory that exists; falls back to the repo-root
/// candidate (the pane then shows "(no document)").
fn resolve_docs_dir() -> PathBuf {
    let mut candidates: Vec<PathBuf> = Vec::new();
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            candidates.push(dir.join("help"));
        }
        if let Some(repo) = exe.ancestors().nth(3) {
            candidates.push(repo.join("help"));
        }
    }
    let repo_help = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(|r| r.join("help"));
    if let Some(p) = repo_help.clone() {
        candidates.push(p);
    }
    for c in &candidates {
        if c.is_dir() {
    ...
    repo_help.unwrap_or_else(|| PathBuf::from("help"))
}
...
pub fn register_class() -> Result<(), super::IGuiError> {
    ...
    let h_instance = unsafe { GetModuleHandleW(None) }
        .map_err(|e| super::IGuiError::Win32(format!("GetModuleHandleW (help): {e}")))?
        .into();
    let cursor = unsafe { LoadCursorW(None, IDC_ARROW) }
        .map_err(|e| super::IGuiError::Win32(format!("LoadCursorW (help): {e}")))?;
    let cls = WNDCLASSEXW {
...
/// Open the Help pane (Help → Documentation / F1). Resolves the docs
/// folder and creates the browser MDI child. Called on the GUI thread
/// from the frame's WM_COMMAND handler.
pub fn open(_frame: HWND, mdi: HWND) {
    // One Help pane at a time: if one is already open, activate it.
    if let Some(raw) = *HELP_HWND.lock().expect("HELP_HWND poisoned") {
    ...
    let title: Vec<u16> = "\u{2042} help".encode_utf16().chain(std::iter::once(0)).collect();
    let _ = create_on_gui_thread(mdi, &title, &path);
}

fn create_on_gui_thread(mdi: HWND, title_utf16: &[u16], path: &str) -> Option<i64> {
...
unsafe extern "system" fn help_wnd_proc(
    ...
    if msg == WM_NCCREATE {
        ...
        if let Ok(mut slot) = HELP_HWND.lock() {
