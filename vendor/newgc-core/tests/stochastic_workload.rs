...
use newgc_core::{
    Generation, HeapHeader, HeapLayout, HeapType, LispLayout, PageHeap, Tag, Word, PAGE_SIZE_CELLS,
    PAYLOAD_MASK,
};
...
fn alloc_large_vector(h: &mut Heap, g: Generation, n_payload: u32, seed: i64) -> Word {
    ...
    unsafe {
        (p.as_ptr() as *mut u64).write(HeapHeader::new(HeapType::Vector, n_payload).raw());
...
fn alloc_vector(h: &mut Heap, g: Generation, len: u32, seed: i64) -> Word {
    ...
    for i in 0..len {
        unsafe {
            set_vector_slot(v, 1 + i as usize, Word::fixnum(seed.wrapping_add(i as i64)));
        }
    }
    v
}
...
unsafe fn count_tree_leaves(root: Word) -> Result<(usize, u32), String> {
    ...
    let depth = depth_word
        .as_fixnum()
        .ok_or_else(|| format!("tree node depth slot not fixnum: {:?}", depth_word))?;
...
fn verify(t: &Tracked) -> Result<(), String> {
    match &t.shape {
        ...
        Shape::Vector { len, seed } | Shape::LargeVector { len, seed } => {
        ...
        Shape::SmallCons { car } => {
            if t.root.tag() != Tag::Cons {
                return Err(format!("SmallCons(id={}): root not Cons-tagged", t.id));
            }
            ...
            match actual.as_fixnum() {
                ...
                None => Err(format!("SmallCons(id={}): car not fixnum", t.id)),
            }
        }
    }
}
...
impl Workload {
    ...
    fn alloc_random(&mut self) -> Word {
        ...
        match kind {
            ...
            2 => {
                ...
                let bytes: Vec<u8> = (0..n).map(|_| (self.rng.next_u64() & 0xff) as u8).collect();
    ...
    fn alloc_random_with_shape(&mut self) -> (Word, Shape) {
        ...
        match kind {
            ...
            _ => {
                ...
                let root = cons(&mut self.heap, Generation::G0, Word::fixnum(car), Word::NIL);
    ...
    fn collect_all_roots(&self) -> Vec<Word> {
        let frame_roots = self.frames.iter().flat_map(|f| f.locals.iter().copied());
    ...
    fn verify_all_tracked(&self) -> Result<(), String> {
        for t in &self.tracked {
            verify(t).map_err(|e| format!("step={}, ops={}: {}", self.step, self.stats.ops, e))?;
        }
        Ok(())
    }
    ...
    fn step(&mut self) {
        ...
        match op_kind {
            ...
            5 => self.major_gc(),
            6 => {
                self.verify_all_tracked().expect("integrity check failed");
...
fn run_stochastic(seed: u64, n_ops: usize, reservation_bytes: usize) {
    ...
    let _ = w
        .heap
        .evacuate_from_word_roots(Generation::Tenured, Generation::Tenured, &mut []);
...
fn run_workload_impl(seed: u64, reservation_bytes: usize, n_ops: usize, enable_full_gc: bool) {
    ...
    eprintln!(
        "seed={seed}, n_ops={n_ops}, enable_full_gc={enable_full_gc}, stats={:?}",
        w.stats
    );
    eprintln!(
        "  gc cycles: {} minor + {} major + {} full; {} integrity checks",
...
fn run_blended(seed: u64, reservation_bytes: usize, n_ops: usize, full_gc_percent: u32) {
    ...
    assert!(
        w.stats.minor_cycles > 5,
        "not enough minor cycles (seed={seed})"
    );
    ...
    assert_eq!(
        w.heap.count_pages_in_gen(Generation::Tenured),
        0,
