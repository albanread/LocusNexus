...
pub enum SurfaceCmd {
    ...
    /// Blit a `w×h` BGRA8 pixel buffer to the pane at `(x, y)`. The **shared
    /// framebuffer** fast path: a Locus program fills a host-owned buffer with
    /// raw `poke`s (the `mem` effect — no per-pixel channel crossing) and then
    /// `present`s it as one command. `pixels` is the snapshot handed across at
    /// present time (one bulk copy), 0xAARRGGBB words = native BGRA bytes
    /// little-endian.
    Blit {
        x: f32,
        y: f32,
...
/// Present a shared framebuffer: replace the pane's batch with a single `Blit`
/// of the `w×h` BGRA buffer and trigger a redraw. The whole frame crosses to
/// the GUI thread as one command (the `pixels` Arc), instead of one command per
/// primitive — the bulk-data fast path behind `Canvas`/`present`.
pub fn present_pixels(child_id: i64, w: u32, h: u32, pixels: Arc<Vec<u32>>) -> bool {
    submit(PaneBatch {
        child_id,
        sequence: next_sequence(),
        flags: 0,
        cmds: vec![SurfaceCmd::Blit { x: 0.0, y: 0.0, w, h, pixels }],
    })
}
