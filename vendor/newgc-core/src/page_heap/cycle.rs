...
impl<L: HeapLayout> PageHeap<L> {
        ...
        F: FnMut(&mut PageEvacuator<'_, L>),
    {
        ...
        self.minors_since_g0_promote = self.minors_since_g0_promote.saturating_add(1);

        let promote_g0 = self.minors_since_g0_promote >= G0_PROMOTION_THRESHOLD;
        ...
        if promote_g0 {
            ...
            self.g0_promotes_since_g1_promote = self.g0_promotes_since_g1_promote.saturating_add(1);
            if self.g0_promotes_since_g1_promote >= G1_PROMOTION_THRESHOLD {
                ...
                let cas = self.evacuate_with_roots(Generation::G1, Generation::Tenured, |e| {
                    visit_roots(e);
                    scan_dirty_cards_as_roots(
                        e,
                        &reservation_cards,
        ...
        F: FnMut(&mut PageEvacuator<'_, L>),
    {
        ...
        let g1_result = self.evacuate_with_roots(Generation::G1, Generation::Tenured, |e| {
            visit_roots(e);
            scan_dirty_cards_as_roots(
                e,
                &reservation_cards,
        ...
        let g0_result = self.evacuate_with_roots(Generation::G0, Generation::G0, |e| {
            visit_roots(e);
            scan_dirty_cards_as_roots(
                e,
                &reservation_cards,
        ...
        let promoted_g1 = g1_result.objects_copied > 0 || g1_result.pages_flipped > 0;
        ...
        F: FnMut(&mut PageEvacuator<'_, L>),
    {
        ...
        let tenured_evac =
            self.evacuate_with_roots(Generation::Tenured, Generation::Tenured, |e| {
                visit_roots(e);
...
mod tests {
    ...
    fn rooted_survivor_promotes_after_threshold_cycles() {
        ...
        for cycle in 1..G0_PROMOTION_THRESHOLD {
            let result = h.collect_minor(|evac| evac.visit(&mut root[0]));
            assert!(!result.promoted_g0, "cycle {cycle}: not yet");
            let addr = (root[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
    ...
    fn promotion_resets_counter_and_starts_new_cohort() {
        ...
        let addr2 = (root2[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        ...
        let addr2 = (root2[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        assert_eq!(h.desc(h.page_of(addr2).unwrap()).generation, Generation::G1);
    }
    ...
    fn g1_promotes_to_tenured_after_threshold_g0_promotions() {
        ...
        for _ in 0..total_minors {
            last_result = Some(h.collect_minor(|evac| evac.visit(&mut root[0])));
        }
        let r = last_result.unwrap();
        assert!(r.promoted_g0, "final cycle promoted G0");
        assert!(r.promoted_g1, "final cycle also cascaded G1 → Tenured");
        let addr = (root[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        let page = h.page_of(addr).unwrap();
        assert_eq!(
            h.desc(page).generation,
    ...
    fn major_promotes_g1_and_collects_g0() {
        ...
        // old_root now in G1.
        let g1_addr = (old_root[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        assert_eq!(
        ...
        assert!(result.promoted_g1, "major reports G1 → Tenured promotion");

        // old_root now in Tenured.
        let t_addr = (old_root[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        assert_eq!(
        ...
        let g_addr = (new_root[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        assert_eq!(
    ...
    fn cascade_reports_nonzero_objects_copied_when_g1_has_data() {
        ...
        for _ in 0..total {
            final_result = Some(h.collect_minor(|evac| evac.visit(&mut root[0])));
        }
    ...
    fn unrooted_g1_cons_reclaimed_on_cascade() {
        ...
        let rooted_g1_addr = (rooted[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        let transient_g1_addr = (transient[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        assert_eq!(
        ...
        let remaining = (G1_PROMOTION_THRESHOLD - 1) * G0_PROMOTION_THRESHOLD;
        for _ in 0..remaining {
            h.collect_minor(|evac| evac.visit(&mut rooted[0]));
        }
        // Now should be in Tenured.
        let rooted_t_addr = (rooted[0].raw() & crate::word::PAYLOAD_MASK) as *const u8;
        assert_eq!(
    ...
    fn g0_and_g1_empty_after_full_collect() {
        ...
        assert_eq!(
            h.count_pages_in_gen(Generation::G0),
            0,
            "G0 empty after full collect"
        );
        assert_eq!(
            h.count_pages_in_gen(Generation::G1),
            0,
            "G1 empty after full collect"
        );
    }
