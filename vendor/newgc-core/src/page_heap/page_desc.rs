...
mod tests {
    ...
    fn n_span_is_one_for_cons_and_boxed() {
        assert_eq!(PageDesc::fresh(Generation::G0, PageKind::Cons).n_span, 1);
        assert_eq!(PageDesc::fresh(Generation::G1, PageKind::Boxed).n_span, 1);
    }
    ...
    fn large_head_and_cont_predicates() {
        ...
        assert!(
            cont.is_large_cont(),
            "fresh Large is a continuation placeholder"
        );
