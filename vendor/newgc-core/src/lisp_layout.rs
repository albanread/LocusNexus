...
impl HeapLayout for LispLayout {
    ...
    fn classify(raw: u64) -> WordKind {
        ...
        match w.tag() {
            ...
            Tag::Cons => WordKind::PointerCons((raw & PAYLOAD_MASK) as *const u8),
            Tag::Symbol | Tag::Vector | Tag::Function | Tag::String => {
                WordKind::PointerHeader((raw & PAYLOAD_MASK) as *const u8)
            }
            Tag::Forward => WordKind::Forwarded((raw & PAYLOAD_MASK) as *const u8),
        }
    }
...
mod tests {
    ...
    fn classify_nil_is_immediate() {
        assert!(matches!(
            LispLayout::classify(Word::NIL.raw()),
            WordKind::Immediate
        ));
    }
    ...
    fn classify_t_is_immediate() {
        assert!(matches!(
            LispLayout::classify(Word::T.raw()),
            WordKind::Immediate
        ));
    }
