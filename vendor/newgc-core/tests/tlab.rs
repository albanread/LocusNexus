...
use newgc_core::{GcCoordinator, Generation, LispLayout, Tag, Word, PAGE_SIZE_CELLS, PAYLOAD_MASK};
...
#[test]
fn tlab_boxed_allocs_are_16_byte_aligned() {
    let coord = Coord::with_reservation(16 * 64 * 1024);
    let mut m = coord.register_mutator();

    for _ in 0..64 {
        let p = m.try_alloc_boxed_in(Generation::G0, 3).expect("alloc");
        assert_eq!(
            p.as_ptr() as usize % 16,
            0,
            "TLAB boxed object must be 16-byte aligned"
        );
    }
}
...
fn start_bits_set_correctly_under_concurrent_alloc() {
    ...
    assert_eq!(
        g0_after, 0,
        "no roots -> G0 fully reclaimed after concurrent alloc"
    );
}
