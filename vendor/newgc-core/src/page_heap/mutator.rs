...
impl<L: HeapLayout> Mutator<L> {
    ...
    fn bump(
        ...
        loop {
            // Fast path: room in the current TLAB. Boxed objects start on
            // even cells, preserving the central allocator's 16-byte
            // alignment invariant for mutator-allocated arrays and vectors.
            let room_cells = self.tlabs[gi][ki].room_cells();
            if room_cells > 0 {
                let cursor = self.tlabs[gi][ki].cursor;
                let cursor_cell = (cursor as usize - self.base_addr) / 8;
                let align_cells = if is_cons { 1 } else { 2 };
                let aligned_cell = align_up_cells(cursor_cell, align_cells);
                let padding_cells = aligned_cell - cursor_cell;
                let needed_cells = padding_cells + n_cells;
                if room_cells >= needed_cells {
                    let ptr = unsafe { cursor.add(padding_cells) };
                    self.tlabs[gi][ki].cursor = unsafe { ptr.add(n_cells) };
                    if is_cons {
                        set_cons_start_bit_at(&self.shared.start_bits, aligned_cell);
                    } else {
                        set_start_bit_at(&self.shared.start_bits, aligned_cell);
                    }
                    self.shared
                        .bytes_alloc_since_gc
                        .fetch_add(needed_cells * 8, Ordering::Relaxed);
    ...
    fn refill(
        ...
        let min_cells = if kind == PageKind::Boxed && min_cells < PAGE_SIZE_CELLS {
            min_cells + 1
        } else {
            min_cells
        };
        ...
        match slab {
            Some((ptr, page_idx, cells)) => {
                let next = ((self.tlabs[gi][ki].next_refill_cells as usize) * 2).min(MAX_TLAB_CELLS)
                    as u32;
        ...
        let result = {
            ...
            let others = self.registry.snapshot();
            {
                ...
                for m in &others {
                    ...
                    {
                        let budget =
                            Duration::from_millis(sp.wait_timeout_ms.load(Ordering::Relaxed));
