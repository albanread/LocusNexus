...
pub extern "C" fn igui_open_child(
    ...
    // `width`/`height` are the requested **client area** in pixels; 0 (either
    // axis) falls back to the Windows default size.
    match super::window::open_child_sized(&title_str, width as i32, height as i32) {
        Some(id) => {
            unsafe { *out_child = id };
            1
        }
...
// ── shared framebuffer (Canvas) — the bulk-data fast path ────────────────────
//
// A Locus program opens a `w×h` BGRA buffer for a pane, writes pixels straight
// into it with `poke32` (the `mem` effect — no boundary crossing per pixel),
// then `present`s it as a single command. Host-owned (the buffer lives here);
// the program receives only its base address (a write capability). Single
// worker thread fills, then presents — so the present-time snapshot reads a
// quiescent buffer (the "copy-on-present" model; no double-buffer needed).

struct CanvasBuf {
    w: u32,
    h: u32,
    /// Two framebuffers, ping-ponged for **zero-copy double buffering**. The
    /// program writes the *back* buffer (`bufs[widx]`); `present` hands the GUI
    /// the *front* (the just-written one) as an `Arc` clone — a refcount bump,
    /// not a pixel copy — and flips `widx` so the next frame is drawn into the
    /// other buffer while the GUI still reads the first.
    bufs: [Arc<Vec<u32>>; 2],
    widx: usize,
}
...
/// Open (or resize) the shared `w×h` BGRA framebuffer for `child_id` and return
/// the **base address** of the back buffer (where the program writes) as an
/// `Int`. The program writes `0xAARRGGBB` words with `poke32` (native BGRA,
/// little-endian) and then calls `igui_canvas_present`. Returns 0 on bad dims.
#[unsafe(export_name = "iGui.CanvasOpen")]
pub extern "C" fn igui_canvas_open(child_id: i64, w: i64, h: i64) -> i64 {
    if w <= 0 || h <= 0 {
        return 0;
    }
    ...
    let buf = map.entry(child_id).or_insert_with(|| CanvasBuf {
        w: 0,
        h: 0,
        bufs: [Arc::new(Vec::new()), Arc::new(Vec::new())],
        widx: 0,
    });
    if buf.w != w || buf.h != h || buf.bufs[0].len() != n {
        buf.w = w;
        buf.h = h;
        buf.bufs = [Arc::new(vec![0u32; n]), Arc::new(vec![0u32; n])];
        buf.widx = 0;
    }
    buf.bufs[buf.widx].as_ptr() as i64
}

/// Present the shared framebuffer for `child_id`: hand the GUI the just-written
/// buffer (an `Arc` clone — no pixel copy), flip to the other buffer, and return
/// the **new back-buffer base address** for the next frame. Animated programs
/// thread that address into the next `render`; a one-shot renderer ignores it.
/// Returns 0 if no canvas is open. (Two buffers — for a producer that can lap
/// the GUI you'd want a third; fine when render time ≥ paint time.)
#[unsafe(export_name = "iGui.CanvasPresent")]
pub extern "C" fn igui_canvas_present(child_id: i64) -> i64 {
    let handoff = {
        let mut guard = CANVAS.lock().expect("CANVAS poisoned");
        guard.as_mut().and_then(|m| m.get_mut(&child_id)).map(|b| {
            let show = b.widx;
            b.widx = 1 - b.widx;
            let front = Arc::clone(&b.bufs[show]);
            let new_base = b.bufs[b.widx].as_ptr() as i64;
            (b.w, b.h, front, new_base)
        })
    };
    match handoff {
        Some((w, h, front, new_base)) => {
            batch_mod::present_pixels(child_id, w, h, front);
            new_base
        }
        None => 0,
    }
...
fn write_event(
    ...
    match ev {
        ...
        IGuiEvent::AnalyzeBuffer { .. } => {
            // Like EvalBuffer: the source rides the Rust-native path
            // (the IDE worker drains the typed variant). C-ABI consumers
            // see the EVAL_BUFFER tag (no payload meaningful to them).
            k = kind::EVAL_BUFFER;
        }
        IGuiEvent::ShowLowering { .. } => {
            // Rust-native path only (the IDE worker renders the dump); C-ABI
            // consumers see the EVAL_BUFFER tag.
            k = kind::EVAL_BUFFER;
        }
...
// ─── IDE-world symbol table (the JIT injection point) ─────────────────
//
// Phase 2a: the IDE host registers these `(export-name, address)` pairs
// with the in-process JIT (via `locus_llvm::jit_run_i64_with_symbols`),
// so a graphical Locus program's `extern "iGui.…"` externs — minted by
// the `IdeGraphics`/`IdeEvent` stdlib boundary and sealed by the
// `Graphics`/`Event` services — resolve to these linked shell functions.
//
// The names are the `export_name`s above (the `iGui.*` C-ABI). The
// boundary `.locus` declares its externs with these exact names. String
// arguments are marshalled on the Locus side into a raw NUL-terminated
// UTF-8 buffer (the `(ptr, len)` CP shape `read_cp_shortstr` already
// expects), so no per-string Rust shim is needed — the numeric draw
// primitives bind directly, `f64` to `f64`.

/// Every `igui_*` GUI C-ABI export the IDE injects into the JIT, as
/// `(symbol-name, function-pointer-as-u64)`. This is the IDE-world's
/// symbol table — the address half of the `Graphics`/`Event` boundary.
/// The host maps these into the `(String, u64)` pairs
/// [`jit_run_i64_with_symbols`](../../locus_llvm) takes.
pub fn ide_symbol_table() -> Vec<(&'static str, u64)> {
    // One entry per export the boundary `.locus` modules bind. Cast each
    // `extern "C" fn(...)` to a thin fn pointer, then to `usize`/`u64`.
    macro_rules! sym {
        ($name:literal, $f:path) => {
            ($name, $f as usize as u64)
        };
    }
    vec![
        // ── window / pane lifecycle ──
        sym!("iGui.OpenChild", igui_open_child),
        sym!("iGui.CloseChild", igui_close_child),
        sym!("iGui.SetTitle", igui_set_title),
        // ── batch builder ──
        sym!("iGui.BeginBatch", igui_begin_batch),
        sym!("iGui.SubmitBatch", igui_submit_batch),
        sym!("iGui.EmitPresentHint", igui_emit_present_hint),
        // ── shared framebuffer (Canvas) ──
        sym!("iGui.CanvasOpen", igui_canvas_open),
        sym!("iGui.CanvasPresent", igui_canvas_present),
        // ── geometry primitives ──
        sym!("iGui.EmitClear", igui_emit_clear),
        sym!("iGui.EmitFillRect", igui_emit_fill_rect),
        sym!("iGui.EmitStrokeRect", igui_emit_stroke_rect),
        sym!("iGui.EmitDrawLine", igui_emit_draw_line),
        sym!("iGui.EmitFillOval", igui_emit_fill_oval),
        sym!("iGui.EmitFillCircle", igui_emit_fill_circle),
        sym!("iGui.EmitStrokeOval", igui_emit_stroke_oval),
        sym!("iGui.EmitStrokeCircle", igui_emit_stroke_circle),
        sym!("iGui.EmitDrawArc", igui_emit_draw_arc),
        // ── text ──
        sym!("iGui.EmitDrawTextRun", igui_emit_draw_text_run),
        // ── system colours ──
        sym!("iGui.SystemColor", igui_system_color),
        // ── events + animation ──
        sym!("iGui.NextEvent", igui_next_event),
        sym!("iGui.SetRedrawRate", igui_set_redraw_rate),
        // ── frame ──
        sym!("iGui.Quit", igui_quit),
    ]
}

#[cfg(test)]
mod ide_symbol_table_tests {
    use super::*;

    #[test]
    fn table_is_nonempty_and_addresses_are_live() {
        let table = ide_symbol_table();
        assert!(
            table.len() >= 15,
            "the IDE symbol table should cover the draw/event surface"
        );
        for (name, addr) in &table {
            assert_ne!(*addr, 0, "`{name}` must resolve to a real address");
        }
    }

    #[test]
    fn the_core_demo_surface_is_present() {
        let names: Vec<&str> = ide_symbol_table().into_iter().map(|(n, _)| n).collect();
        for needed in [
            "iGui.OpenChild",
            "iGui.BeginBatch",
            "iGui.SubmitBatch",
            ...
            assert!(names.contains(&needed), "missing JIT symbol `{needed}`");
        }
    }
}
