...
use newgc_core::{
    Generation, HeapHeader, HeapType, LispLayout, PageHeap, Tag, Word, G0_PROMOTION_THRESHOLD,
    G1_PROMOTION_THRESHOLD, PAYLOAD_MASK,
};
...
fn alloc_string_n(h: &mut Heap, byte: u8, n: usize) -> Word {
    let p = h
        .try_alloc_boxed_in(Generation::G0, 1 + n)
        .expect("string alloc");
...
fn gap010_external_root_rewritten_after_major_compaction() {
    ...
    let mut below: Vec<Word> = (0..150)
        .map(|i| alloc_string(&mut h, (i & 0x7f) as u8))
        .collect();
    let mut keep = alloc_string(&mut h, 0xAB);
    let mut above: Vec<Word> = (0..150)
        .map(|i| alloc_string(&mut h, 0x40 | (i & 0x3f) as u8))
        .collect();
    ...
    assert!(
        ...
        "keep corrupted by the promoting collection"
    );
...
fn gap010_minor_promotion_of_surviving_string() {
    ...
    let n_minors = (G0_PROMOTION_THRESHOLD as usize) * (G1_PROMOTION_THRESHOLD as usize) + 4;
...
fn gap010_card_scan_must_not_treat_byte_payload_as_pointer() {
    ...
    assert_ne!(
        page_gen(&h, keep),
        Generation::G0,
        "keep should be promoted out of G0"
    );
    ...
    let payload0_after = unsafe { *((keep.raw() & PAYLOAD_MASK) as *const u64).add(1) };
    eprintln!(
